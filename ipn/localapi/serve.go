...
func (h *Handler) serveServeConfig(w http.ResponseWriter, r *http.Request) {
	...
	case httpm.POST:
		...
		// require a local admin when setting a path handler, serving a Unix
		// domain socket, or reading a TLS private key
...
func authorizeServeConfigForGOOSAndUserContext(goos string, configIn *ipn.ServeConfig, h *Handler) error {
	...
	if !configIn.HasPathHandler() && !configIn.IsServingUnixAny() && !configIn.HasManualCertificate() {
		return nil
	}
	...
	switch goos {
	case "windows":
		return errors.New("must be a Windows local admin to serve a path, Unix socket, or manual TLS certificate")
	case "linux", "darwin", "illumos", "solaris":
		return errors.New("must be root, or be an operator and able to run 'sudo tailscale' to serve a path, Unix socket, or manual TLS certificate")
	default:
